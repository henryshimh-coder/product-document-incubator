...
- 验收提交 SHA：`2b02b56415275dacc29d9db2c92302d449212e10`（分支 `codex/v4-remediation`，提交信息 `fix: bind publish evidence metadata to verified material`）
- 换证说明：本轮以 v4 最终整改代码重新执行全部浏览器验收并覆盖 v3 轮（`b3845d5`）截图；v4 仅收紧发布闸证据元数据校验，页面行为不变，下方测量结果为本轮实测。
...
3. 六节点顺序（两视口一致，实测卡片文本）：原始资料（SRC-LLD-BASE，已入库）→ 结构化知识（RULE-LLD-001，生效中）→ 问题（ISSUE-74C2DDBE13743AEA87E0，已决定）→ 人工决定（DECISION-7A1892E80B224A4187C19484763A5C61，接受迭代）→ 变更单（CHANGE-623DCEA8CC714E00AF4FB1F1205E3996，已发布，目标 LLD-724_2）→ 生效基线（BASE-LLD-724_2 / LLD-724_2，生效中）。发布前同页仅 5 节点并提示「缺失环节：生效基线」属预期行为。
4. 移动端六节点：每卡 `x=20 w=350 right=370 ≤ 390`，纵向 y=323…1563 堆叠不越界。
5. 桌面端六节点：横向排列 y≈354，最高卡 bottom=515 ≤ 1024，六节点首屏全部可见。

## 失败→回退→重试验证（真实篡改测试）

1. 篡改：`/tmp/t10t11_browser/data/source_archive/LLD/SRC-LLD-BASE/当前产品方案.md` 追加 41 字节（原文 19731 字节，先完整备份）。
...
T10（原子发布）与 T11（六节点追溯）在真实浏览器、真实隔离工程、真实篡改攻击下行为符合 v3/v4 修改建议预期：发布闸前移至创建临时目录之前、证据元数据（citation 身份/版本/locator/excerpt）全字段绑定、来源完整性失败可安全回退并免重复批准重试、六节点链路在双视口完整有序且无横向溢出。本轮截图以 v4 最终代码提交 `2b02b56` 重新拍摄，覆盖 v3 轮旧图。
